...
import org.apache.spark.sql.catalyst.plans.logical.statsEstimation.StatsSqlStatus.StatsSqlStatus
...
class StatsSqlGeneration(schema: StructType,
  ...
  protected val validFilters: Seq[Expression] =
    filters.filter(f => StatsSqlGeneration.validateFilterExpression(f))
...
object StatsSqlGeneration {
  ...
  def validateFilters(filters: Seq[Expression]): StatsSqlStatus = {
    var invalidCount = 0
    var validCount = 0
    for (f <- filters) {
    ...
    if (invalidCount == 0 && validCount > 0) {
      StatsSqlStatus.FullyValid
    } else if (invalidCount > 0 && validCount > 0) {
      StatsSqlStatus.PartiallyValid
    } else if (filters.isEmpty) {
      StatsSqlStatus.FullyValid
    } else {
      StatsSqlStatus.Invalid
    }
  }
