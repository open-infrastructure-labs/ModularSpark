...
import org.apache.spark.sql.catalyst.plans.logical.{ColumnStat, ExposesMetadataColumns, LeafNode, LogicalPlan, Statistics}
...
import org.apache.spark.sql.connector.read.{Scan, SupportsReportStatistics, Statistics => V2Statistics}
...
object DataSourceV2Relation {
  ...
  def transformV2Stats(
    ...
    val attributeStats: AttributeMap[ColumnStat] = {
      if (v2Statistics.isInstanceOf[ReadStatsWithAttributes]) {
        val readStats = v2Statistics.asInstanceOf[ReadStatsWithAttributes]
        readStats.attributeStats
      } else {
        AttributeMap(Nil)
      }
    }
