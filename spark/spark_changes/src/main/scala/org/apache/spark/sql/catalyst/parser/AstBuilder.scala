...
import org.apache.spark.sql.catalyst.parser.SqlBaseParser.{NewFuncTableContext, _}
...
import org.apache.spark.sql.execution.python.PythonReader
...
class AstBuilder extends SqlBaseParserBaseVisitor[AnyRef] with SQLConfHelper with Logging {
   ...
   * }}}
   */
  override def visitNewFuncTable(ctx: NewFuncTableContext): AnyRef = withOrigin(ctx) {
    val from = OneRowRelation().optional(ctx.newFunc().fromClause) {
      visitFromClause(ctx.newFunc().fromClause)
    }
   //  val query = plan(ctx.queryTerm).optionalMap(ctx.queryOrganization)(withQueryResultClauses)
    val name = Option(ctx.newFunc().param).map(string)
    val func: Expression = PythonReader.getPythonFunc(name.get)
    NewFunction(
      Option(ctx.newFunc().param).map(string),
      Option(ctx.newFunc().number).map(_.getText.toDouble),
      func,
      from)
  }
  /**
   ...
   * }}}
   */
  override def visitNewFuncTable1(ctx: NewFuncTable1Context): AnyRef = withOrigin(ctx) {
    val from = OneRowRelation().optional(ctx.newFunc().fromClause) {
      visitFromClause(ctx.newFunc().fromClause)
    }
    //  val query = plan(ctx.queryTerm).optionalMap(ctx.queryOrganization)(withQueryResultClauses)
