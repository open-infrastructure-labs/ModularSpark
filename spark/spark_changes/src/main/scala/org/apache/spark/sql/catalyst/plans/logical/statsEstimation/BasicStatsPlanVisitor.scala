...
object BasicStatsPlanVisitor extends LogicalPlanVisitor[Statistics] {
  ...
  override def visitFilter(p: Filter): Statistics = {
    // If the estimate is not available for the extended filter estimate,
    // then fall back to the standard filter estimate.
    val stats = FilterEstimationExt(p).estimate
    if (stats.isDefined) {
      stats.getOrElse(fallback(p))
    } else {
