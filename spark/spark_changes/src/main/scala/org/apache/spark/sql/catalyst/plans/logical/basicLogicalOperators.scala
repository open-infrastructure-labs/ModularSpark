...
case class NewFunction(param: Option[String],
...
//      AttributeReference("plan", StringType, nullable = true)())
  ...
  final override val nodePatterns: Seq[TreePattern] = Seq(NEW_FUNCTION)
  ...
  override protected def withNewChildInternal(newChild: LogicalPlan): NewFunction =
    copy(child = newChild)
}