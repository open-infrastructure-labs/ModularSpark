 ...
 */
class WriteBufferStream(schema: StructType,
                        batchSize: Int,
                        outputStream: OutputStream,
                        pool: WriteBufferPool)
    extends DataStreamItem {
  ...
  private def batchSizeForType(dataType: DataType): Int = {
    dataType match {
      ...
      case StringType => batchSize * 8 // ServerHeader.stringLength
    }
  }
  private def sizeForType(dataType: DataType): Int = {
    dataType match {
      ...
      case LongType => 8
      case DoubleType => 8
      case StringType => ServerHeader.stringLength
    }
  }
  private val header = getHeader
  def getHeader: Array[ByteBuffer] = {
    val h = schema.fields.map(_ => ByteBuffer.allocate(ServerHeader.bytes))
    schema.fields.zipWithIndex.map(s => {
      s._1.dataType match {
        case LongType =>
          h(s._2).putInt(ServerHeader.Offset.dataType,
            ServerHeader.DataType.LongType.id)
          h(s._2).putInt(ServerHeader.Offset.typeSize,
            ServerHeader.Length.Long)
        case DoubleType =>
          h(s._2).putInt(ServerHeader.Offset.dataType,
            ServerHeader.DataType.DoubleType.id)
          h(s._2).putInt(ServerHeader.Offset.typeSize,
            ServerHeader.Length.Double)
        case StringType =>
          h(s._2).putInt(ServerHeader.Offset.dataType,
            ServerHeader.DataType.ByteArrayType.id)
          h(s._2).putInt(ServerHeader.Offset.typeSize, 0)
      }
    })
    h
  }
    ...
    dataType match {
      ...
      case StringType =>
        (row: SpecializedGetters, ordinal: Int) =>
          ...
          if (numBytes >= bufferLength(ordinal) - ServerHeader.stringLength) {
...
//        logger.trace(s"col $i rows $rows $name")
        ...
        header(i).putInt(ServerHeader.Offset.dataLen,
          dataLen)
        header(i).putInt(ServerHeader.Offset.compressedLen,
