 ...
 *
 */
class DataStreamer extends java.lang.Thread {
  val queue: ArrayBlockingQueue[DataStreamItem] =
    new ArrayBlockingQueue[DataStreamItem](16)

  def enqueue(item: DataStreamItem): Unit = {
...
object DataStreamer {
   ...
   */
  def apply(threads: Int): Array[DataStreamer] = {
    Array.fill[DataStreamer](threads)(new DataStreamer)
  }
}
