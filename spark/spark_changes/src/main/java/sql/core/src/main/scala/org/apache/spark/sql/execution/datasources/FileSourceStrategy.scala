...
object FileSourceStrategy extends Strategy with PredicateHelper with Logging {
  ...
  private def genBucketSet(
    ...
    logInfo {
      s"changedCode:  Pruned ${numBuckets - numBucketsSelected} out of $numBuckets buckets."
    }
  ...
  def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    case ScanOperation(projects, filters,
      ...
      logInfo(s"changedCode: Pushed Filters: ${pushedFilters.mkString(",")}")

      // Predicates with both partition keys and attributes need to be evaluated after the scan.
      val afterScanFilters = filterSet -- partitionKeyFilters.filter(_.references.nonEmpty)
      logInfo(s"changedCode: Post-Scan Filters: ${afterScanFilters.mkString(",")}")
      ...
      logInfo(s"changedCode: Output Data Schema: ${outputSchema.simpleString(5)}")
